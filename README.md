# Kourosh Afsharpour

Real-time simulation and systems programmer focused on physically grounded models,
engine-level development, and interactive evaluation under real-time constraints.

## About

My work centers on real-time simulation as a foundation for interactive systems,
games, and research-oriented evaluation. I am particularly interested in
physics-based modeling, numerical stability, and building transparent simulation
pipelines that remain controllable and interpretable under real-time constraints.

My background spans engine-level development in Unity and Unreal Engine,
research-oriented simulation systems, and shipped interactive applications.

## Projects

### Vehicle Physics for Real-Time Simulation (Unreal Engine, C++)
Research-oriented vehicle dynamics system developed from first principles.

- Custom raycast-based wheel and suspension model
- Tire forces modeled using Pacejka Magic Formula 5.2 with transient slip dynamics
- Fully custom drivetrain with physically interpretable torque propagation
- Designed for numerical stability under variable frame rates (down to ~15 FPS)

This system serves both as an interactive simulation platform and as a research
testbed for studying physically grounded dynamics under real-time constraints.

**Links**
- Vehicle Physics – Writing Sample (PDF): [https://drive.google.com/file/d/1AJyO24DnAH7WrPG2FEDiHwNMUsd3QzBW/view?usp=sharing]

**Videos**

[![Vehicle Physics Demo](https://img.youtube.com/vi/kpArE7zeihY/0.jpg)](https://www.youtube.com/watch?v=kpArE7zeihY)
[![Vehicle Physics Demo](https://img.youtube.com/vi/TXzRp0bGmJU/0.jpg)](https://youtu.be/TXzRp0bGmJU?si=79PjLW1gbN5gx8kt)

### Airplane Flight Simulator (Unity, C#)
Open-world arcade flight game released on iOS, Android, and Fire OS.

- Role: Lead / main programmer
- Implemented aircraft physics, mission systems, and core gameplay logic
- Designed and optimized large-scale environments for mobile hardware
- Implemented asynchronous scene loading, asset-bundle-based world management,
  and custom LOD solutions for dense environments

This project emphasized building robust systems that balance physical behavior,
player interaction, and strict real-time performance constraints.

**Links**
- Store page (if available)

**Gameplay videos**

[![Airplane Flight Simulator](https://img.youtube.com/vi/CWr0VuZJQSo/0.jpg)](https://youtu.be/CWr0VuZJQSo?si=mIhMsSEz8QUCk1XC)
[![Airplane Flight Simulator](https://img.youtube.com/vi/CWr0VuZJQSo/0.jpg)](https://youtu.be/03bmqtihf94?si=TxCJQpo96j9KY915)

Additional context on my research interests and background is available in my
statement of purpose and writing samples.

## Links

- GitHub: [https://github.com/Kouroshx99](https://github.com/Kouroshx99)
- Email: [kafsharpour@outlook.com](mailto:kafsharpour@outlook.com)
